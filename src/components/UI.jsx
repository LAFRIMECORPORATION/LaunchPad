...
export function Badge({ children, color = "blue" }) {
  const cls =
    BADGE_COLOR_MAP[color] || BADGE_COLOR_MAP[children] || "badge-gray";
...
export function Tag({ children, active = false, onClick }) {
  return (
    <span className={`tag${active ? " active" : ""}`} onClick={onClick}>
      {children}
    </span>
  );
}
...
export function ProgressBar({ value, max, size = "normal", showMeta = true }) {
  ...
  return (
    <div>
      <div
        className={`progress-bar${size === "thin" ? " thin" : size === "thick" ? " thick" : ""}`}
      >
    ...
    >
      ...
      <div style={{ padding: compact ? "14px 16px" : "18px 20px" }}>
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          >
            <span style={{ color: "var(--text-main)" }}>{pct}% financé</span>
            <span style={{ color: "var(--text-muted)" }}>
              {Math.round(project.raised / 1000000)}M /{" "}
              {Math.round(project.goal / 1000000)}M XAF
            </span>
          </div>
          <div
            style={{
  ...
  return (
    <span
      style={{
        ...
        background:
          "linear-gradient(135deg, rgba(91,115,245,.12), rgba(139,92,246,.12))",
  ...
  return (
    <aside className="sidebar">
      ...
      {currentUser &&
        ...
        currentUser.kycValidated && (
          <div
            className="kyc-sidebar-verified"
            style={{
              display: "flex",
              gap: 8,
              padding: "10px 14px",
...
export function KycAlert({ compact = false }) {
  ...
  if (!currentUser || currentUser.kycValidated || currentUser.role === "admin")
    return null;
